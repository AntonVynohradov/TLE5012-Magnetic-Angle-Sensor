...
public:
    ...
    {  //!< \brief Safety word bit setting
        ...
        uint8_t CORELIB_CRC;  //!< \brief bits 7:0 Status ADC Test
        ...
        {
            return (RESP == 0x7 ? TLE5012B_S3 : (RESP == 0xB ? TLE5012B_S2 : (RESP == 0xD ? TLE5012B_S1 : TLE5012B_S0)));
        }
    ...
    /*!
     * Reads the block of _registers from addresses 08 - 0F in order to figure out the CORELIB_CRC.
     * ATTENTION: You need a memory chunk of unit16_t * CORELIB_CRC Registers + 1 * uint16_t for the safety word.
     * @return CORELIB_CRC error type
     */
     ...
     * the CORELIB_CRC, and their values are stored in the private component of the class, _registers.
     * @param [out] data where the data received from the _registers will be stored
     * @return CORELIB_CRC error type
     */
     ...
     *        so that we don't need to read all the register again to calculate the CORELIB_CRC
     * @return CORELIB_CRC error type
     */
    errorTypes writeToSensor(uint16_t command, uint16_t dataToWrite, bool changeCRC);

    /*!
     * This function is used in order to update the CORELIB_CRC in the register 0F(second byte)
     * @param [in] dataToWrite the new data that will be written to the register
     * @return CORELIB_CRC error type
     */
    ...
    /*!
     * Standard function used for updating the CORELIB_CRC
     * @param [in] dataToWrite the new data that will be written to the register
     * @return CORELIB_CRC error type
     */
...
protected:
    ...
    uint16_t _registers[CRC_NUM_REGISTERS + 1];  //!< \brief keeps track of the values stored in the 8 _registers, for which the CORELIB_CRC is calculated

    /*!
     * This function is called each time any register in the
     * range 08 - 0F(first byte) is changed. It calculates the new CORELIB_CRC
     ...
     * the CORELIB_CRC such that the data received is valid
     * @param safety register with the CORELIB_CRC check data
